...
const DYNAMIC_PROGRAM_MAP = [
  { keywords: [/מדעי המחשב|תוכנה|סייבר|coding|code|web|תוכניתן|פיתוח|דאטה|חומרה|הייטק|סיסטם|cyber/i], id: 'cs', name: 'מדעי המחשב', category: 'מדעי המחשב' },
  ...
  { keywords: [/רפואה|סיעוד|קלינאות|ריפוי|פיזיותרפיה/i], id: 'medicine', name: 'רפואה ומקצועות הבריאות', category: 'רפואה' },
  { keywords: [/משפטים|חוק/i], id: 'law', name: 'משפטים', category: 'משפטים' },
  { keywords: [/מנהל עסקים|עסקים|ניהול|חשבונאות|רואה חשבון/i], id: 'business', name: 'מנהל עסקים', category: 'מנהל עסקים' },
  { keywords: [/כלכלה/i], id: 'economics', name: 'כלכלה', category: 'כלכלה' },
  { keywords: [/עיצוב|אמנות|תקשורת חזותית|משחק|קולנוע|מוסיקה|מוזיקה|מחול|תיאטרון|צילום/i], id: 'graphic_design', name: 'עיצוב ואומנויות', category: 'עיצוב' },
];
...
for (const record of mondayAdmissionsEvidence) {
  ...
  let matches = DYNAMIC_PROGRAM_MAP.filter((prog) => {
    const searchable = `${record.itemName} ${record.displayName} ${record.tags.join(' ')} ${record.decisionReason}`.toLowerCase();
  ...
  for (const prog of matches) {
    ...
    dynamicPrograms.push({
      ...
      admissionType:
        record.publicBucket === 'open_admission'
          ? ('open_admission' as any)
          : 'requirements',
