...
export function mergeUserProfileDraft(
  ...
  return {
    ...
    ...(mergeAcademicScores(baseScores, draftScores) ? { academicScores: mergeAcademicScores(baseScores, draftScores) } : {}),
    savedProgramIds: Array.from(new Set([...(base.savedProgramIds ?? []), ...(draft.savedProgramIds ?? [])])),
  };
}
